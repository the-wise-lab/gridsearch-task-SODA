...
var createGrid = function (
    ...
    for (var i = 0; i < number; i++) {
        // KW: loop through rows
        for (var j = 0; j < number; j++) {
            //KW: loop through columns (or other way around)

            var g = document.createSvg("g");
            g.setAttribute(
                "transform",
                ["translate(", i * size, ",", j * size, ")"].join("")
            ); // KW: moves over to the place where next tile should be
    ...
    svg.addEventListener(
        ...
        "click",
        function (e) {
            // KW: e is the MouseEvent that gives infos on the button click

            if ((outcomeOpen == false) & (training_clickable == true)) {
                //KW: (unsure) outcomeOpen: total win of trial not yet decided (can still click), training_clickable: can click
                ...
                // Add noise
                var nFishThisTrial =
                    targetElement.parentElement.firstElementChild.nfish + noise;
                ...
                if (training_iteration >= 0) {
                    if (training_iteration == 0) {
                        // KW: after the first click of the training the grid becomes white, one can't click anymore but has to click the arrow
                        training_clickable = false;
                        window.setTimeout(function () {
                            document.getElementById("ocean").style.opacity =
                                "40%";
                        }, 1500);
                        console.log(arrow);
                    ...
                    training_clicks += 1;
                    if ((training_clicks > 5) & (training_iteration == 3)) {
                        //KW: after the 5th training click I ALWAYS find 39 fish thus kraken
                        nFishThisTrial = 39;
                    } else if (
                        (nFishThisTrial <= 50) &
                        (training_iteration <= 3)
                    ) {
                        //KW: makes sure I don't find the kraken before 6th click
                ...
                targetElement.parentElement.firstElementChild.textContent =
                    nFishThisTrial; // KW: retrieve fish in clicked tile
                if (box.nFishHistory !== undefined) {
                ...
                if (nFishThisTrial > threshold) {
                    outcome.getElementsByClassName(
                        "textcontainer"
                    ).textcontainer.textContent =
                        "You caught " + nFishThisTrial + " fish!";
                    outcome.getElementsByClassName(
                        "textcontainer"
                    ).textcontainer.innerHTML +=
                        '<br><br><img src="assets/fish.svg" width=100%>';
                    ...
                    score.innerHTML =
                        ...
                        "<br>Clicks left: " +
                        nclicks +
                        "<br><font color='#9c9c9c'>Total score: " +
                        totalScore +
                        "</font>";
                    if (nclicks > 0) {
                        //KW: outcome box go away after 1s
                        setTimeout(function () {
                        ...
                        setTimeout(function () {
                            ...
                            outcome.getElementsByClassName(
                                "textcontainer"
                            ).textcontainer.textContent = "End of block!";
                            outcome
                                .getElementsByClassName("textcontainer")
                ...
                // IF THE KRAKEN IS FOUND
                    ...
                    outcome.getElementsByClassName(
                        ...
                        nFishThisTrial + " fish! You found the Kraken!";
                    outcome.getElementsByClassName(
                        "textcontainer"
                    ).textcontainer.innerHTML +=
                        '<br><br><img src="assets/kraken.svg" width=40%>';
                    ...
                    var scoreInterval = setInterval(function () {
                        ...
                        score.innerHTML =
                            "<font color='#bf0000'>Score this block: " +
                            scoreNumber +
                            "</font><br>Clicks left: " +
                            nclicks +
                            "<br><font color='#9c9c9c'>Total score: " +
                        ...
                        if (scoreNumber == 0) {
                            ...
                            setTimeout(function () {
                                if (currentBlock == 1) {
                                    // if this was the instructions then make the continue button better visible
                                    $(ocean).hide();
                                    $(krakenPresent).hide();
                                } else {
                                    outcome.getElementsByClassName(
                ...
                boat.getElementsByClassName("boatImg")[0].style.width =
                    400 / number + "px";
    ...
    svg.addEventListener("mouseover", function (e) {
        ...
        if (targetElement.nFishHistory !== undefined) {
            if (targetElement.nFishHistory.length) {
                //KW: if there are fish in history then display history otherwise disp not fished here before
    ...
    svg.addEventListener("mouseout", function (e) {
        ...
        if (targetElement.getAttribute("data-click") != null) {
            // KW: if target was clicked in the process then make sure it stays highlighted
            targetElement.setAttribute("fill-opacity", 0.3);
        } else {
    ...
    button.onclick = function () {
        if (training_done) {
            ...
            if (currentBlock - 1 < blocks.length) {
                //KW: if still has blocks to go, empty trial data arrays and run the task again (-2 bc 1 block for intro and 1 for bonus)
                if (understood) {
                    trialData = { xcollect: [], ycollect: [], zcollect: [] };
                    if (currentBlock == 6) {
                        runBonus(grids[envOrder[currentBlock - 1]], 50);
                    } else {
                        runTask(
                            grids[envOrder[currentBlock - 1]],
                            50,
                            blocks[currentBlock - 1]
                        ); // current block starts from 1, 0 is training
                    }
                } else {
                    // disp comprehension questions
                    startComprehension();
                    page6[comprehensionAttempts - 1] =
                        (Number(new Date()) - time6) / 60000;
                }
            } else {
                //KW: if this was the last block
                ...
                endButton.onclick = function () {
                    window.location.href =
                        "questionnairesPost.html?" +
                        "PROLIFIC_PID=" +
                        subjectID +
                        "&UID=" +
                        uid +
                        "&STUDY=" +
                        studyID; // This passes the subjectID, Firebase UID, and Study ID to the next page
                };
                ...
                newBlock.innerHTML =
                    "End of task!<br><br>You caught " +
                    totalScore +
                    " fish, and won $" +
                    bonusPayment.toFixed(2) +
                    "!";
...
function runTraining(numbergrid, training_iteration) {
    ...
    setTimeout(function () {
        //KW: makes the text "next block" be displayed on the grid for 1s then disappear.
        newBlock.style.opacity = 0;
    }, 1000);
    setTimeout(function () {
        //KW: this removes the next block announcement box. Why was it really here in the first place?
...
function runTask(numbergrid, threshold, kraken) {
    ...
    container.appendChild(
        createGrid(11, 10, numbergrid, threshold, false, kraken)
    );
...
function createQuestion(questionnaireName, questionData) {
    // function from Toby's questionnaire code
    ...
    f.setAttribute(
        "id",
        questionnaireName.concat("_" + questionData.qNumber.toString())
    );
    ...
    for (i = 0; i < questionData.labels.length; i++) {
        ...
        c.type = "radio";
        c.id = questionnaireName
            .concat(questionData.qNumber.toString())
            .concat("answer".concat(i.toString()));
        c.name = "question";
        c.value = i;
...
var createComprehension = function () {
    var q1Data = {
        ...
        labels: [
            "To find the kraken.",
            "To click on every square at least once.",
    ...
    var q2Data = {
        ...
        labels: [
            "You will get approximately the same number of fish (+-2).",
            "The number of fish you get will gradually decrease.",
            "You will only get fish the first time, afterwards the fish in that square are gone.",
        ],
    };
    ...
    var q3Data = {
        ...
        labels: [
            ...
            "The lower half of the ocean has more fish.",
            "The number of fish in adjacent tiles is similar.",
        ],
    };
    ...
    var q4Data = {
        ...
        labels: [
            ...
            "The round is over and you lose all the fish you collected in that round.",
    ...
    var q5Data = {
        ...
        labels: [
            "When you click a square with less than 50 fish.",
            "At any moment, you have no control over this.",
            "When you click a square with less than 50 fish on a round where it is written that the kraken is nearby over the ocean.",
        ],
    };
    ...
    var q6Data = {
        ...
        labels: [
            "No, each block is about the same ocean.",
            "Yes, each block is a completely new ocean.",
            "The oceans only change a little bit between blocks.",
        ],
    };
...
function checkComprehension() {
    ...
    for (i = 0; i < inputs.length; i++) {
        if (inputs[i].id.length > 0) {
            ...
            var checked = inputs[i].querySelector(
    ...
    if (incomplete.length > 0) {
        $("html, body").animate(
            {
                // go to first missed items
                scrollTop:
                    $(document.getElementById(incomplete[0])).offset().top -
                    100,
            },
            400
        );
        ...
        if (incomplete.length > 1) {
            // if you missed more than one item

            for (i = 0; i < incomplete.length - 1; i++) {
                // loops through all missed questions and attaches an event listener to each of them

                $(document.getElementById(incomplete[i]))
                    .children()
                    .click(function (e) {
                        var target =
                            e.target.parentElement.parentElement.parentElement
                                .id; // name of the given question
                        ...
                        var nextMiss = document.getElementById(
                            incomplete[n + 1]
                        );
                        $("html, body").animate(
                            {
...
function startComprehension() {
    ...
    instructionHeading.innerHTML =
        "<h2>Please answer some questions to see whether you understood everything correctly.</h2>";
    $(document.getElementById("quizContainer")).show();
    $(document.getElementById("quiz")).show();
    if (comprehensionAttempts == 1) {
        // if this is the first attempt, create the questions
        createComprehension();
    } else {
        // if it isnt, no need to create them but do need to show the button that was previously hidden
        $(document.getElementById("submitContainer")).show();
...
var init = function () {
    ...
    studyID = "data"; // This seems to be a hardcoded override
    ...
    credit.innerHTML =
        '<a href="http://www.freepik.com">Images designed by macrovector / Freepik</a>';
    mainSection.appendChild(credit);

    var instructions = document.getElementById("instructions");

    searchHistory = { xcollect: [], ycollect: [], zcollect: [] };
    trialData = { xcollect: [], ycollect: [], zcollect: [] };

    // Define createSvg and create common DOM elements needed by both paths
    ...
    // Default text, might be overwritten by specific outcomes
    ...
    if (skipInstructions) {
        ...
        var krakenPresent = document.getElementById("krakenPresent");
        if (krakenPresent) $(krakenPresent).show();

        // Initialize arrow when skipping instructions
        ...
        $.getJSON("assets/sample_grid.json")
        ...
        // Original instruction setup and training flow
        ...
        instructionsArray.push(
            "You will have to work out which locations are likely to contain the most fish in order to maximise your score.<br><br>" +
        ...
        instructionsArray.push(
            "Areas where the Kraken lurks will have very few fish as it has started to eat them.<br><br>" +
        ...
        instructionsArray.push(
            "You may have just found the kraken in a place where you didn't expect it - for this example we gave you a low number of fish after 5 clicks, no matter where you clicked. " +
                "In the real game, nearby places in the sea will have similar numbers of fish.<br><br>"
        );

        instructionsArray.push(
            "Half-way through the game there will be a <strong>bonus round</strong>.<br/>That means that after a few clicks, 5 random location will turn red one after the other and you will be asked </br> " +
                ...
                "There will be " +
                blocks.length +
                " blocks, and you get 10 clicks within each block.<br><br>" +
        ...
        var instructionContainer = document.createElement(
            "instructionContainer"
        );
        ...
        instructionText.innerHTML = instructionsArray[0];
        ...
        arrow.addEventListener("mouseup", function () {
            ...
            if (training_iteration == 1) {
                ...
                var krakenPresent = document.getElementById("krakenPresent");
                var oceanElement = document.getElementById("ocean");
                if (oceanElement)
                    oceanElement.setAttribute(
                        "style",
                        "box-shadow: 0px 0px 40px #f00;"
                    );
                if (krakenPresent) krakenPresent.style.visibility = "visible";
                instructionText.classList.add("fade");
            ...
            if (training_iteration == 2) {
                ...
                training_clickable = true;
                var oceanElement = document.getElementById("ocean");
                if (oceanElement) oceanElement.style.opacity = "100%";
                instructionText.classList.add("fade");
                instructionText.innerHTML = instructionsArray[3];
                setTimeout(function () {
                    instructionText.classList.toggle("fade");
                    training_iteration = 3;
                    if (arrow) arrow.style.opacity = 0.1;
                }, 500);
            }
            ...
            if (training_iteration == 5) {
                ...
                if (instructionsElement && button)
                    instructionsElement.appendChild(button);
                setTimeout(function () {
...
var createBonusGrid = function (number, size, numbergrid) {
    ...
    $(svg).mouseout(function (e) {
        ...
        fishHistory.style.opacity = 0; // stop showing the fish history
        if (targetElement.getAttribute("fill") == "white") {
            // this makes sure that red bonus tile stays untouched
            if (targetElement.getAttribute("data-click") != null) {
                // KW: if target was clicked in the process then make sure it stays highlighted
                targetElement.setAttribute("fill-opacity", 0.3);
            } else {
...
var getRandomStimuli = function (n) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        while (!found) {
            ...
            if (!previouslySelected && !isArrayInArray(randomStimuli, item)) {
                //Also check that it's not already been added
...
function startBonusRound(totalBonusRounds) {
    ...
    tile = document.getElementById(
        "square-" +
            (bonusStimuli[bonusCounter][0] * 11 + bonusStimuli[bonusCounter][1])
    );
    ...
    submit.onclick = function () {
        // had to copy the submitBonusJudgment function in here bc it wouldn't work otherwise
        // make confidence either the value of the confidence slider or null
        var confidence =
            confidenceSlider != null ? confidenceSlider.value : null;
        ...
        var z =
            grid[bonusStimuli[bonusCounter][0]][bonusStimuli[bonusCounter][1]];
        ...
        if (bonusCounter + 1 <= totalBonusRounds) {
            //If more rounds remaining, +1 bc bonus counter starts at 0
            change(
                "krakenPresent",
                "Bonus Round: " + (bonusCounter + 1) + " of " + totalBonusRounds
            );
            // reset inputs
            ...
            tile = document.getElementById(
                "square-" +
                    (bonusStimuli[bonusCounter][0] * 11 +
                        bonusStimuli[bonusCounter][1])
            );
            tile.setAttribute("fill", "#FF2020");
            tile.setAttribute("fill-opacity", 0.8);
        } else {
            //bonus round over; start forced choice
            ...
            for (var i = 0; i < totalBonusRounds; i++) {
                //Loop through bonus stimuli
                tile = document.getElementById(
                    "square-" + (bonusStimuli[i][0] * 11 + bonusStimuli[i][1])
                );
                tile.setAttribute("fill", "#FF2020");
                tile.setAttribute("fill-opacity", 0.8);
            }
            change(
                "krakenPresent",
                '<b>Which one of these <span style = "color:red">red</span> squares will you fish from?</b>'
            );
            ...
            svg.addEventListener("click", function (e) {
                ...
                if ((outcomeOpen == false) & (training_clickable == true)) {
                    ...
                    for (var i = 0; i < totalBonusRounds; i++) {
                        ...
                        if (
                            arraysEqual(
                                [targetElement.xpos, targetElement.ypos],
                                [bonusStimuli[i][0], bonusStimuli[i][1]]
                            )
                        ) {
                            validChoice = true;
                            break;
                    ...
                    if (validChoice || nclicks < 5) {
                        // if already chose 1 of the bonus tiles then also go here
                        ...
                        if (nclicks > 0) {
                            ...
                            if (nclicks == 4) {
                                // not 5 bc already subtracted 1 from counter
                                for (var i = 0; i < totalBonusRounds; i++) {
                                    //Loop through bonus stimuli
                                    if (
                                        arraysEqual(
                                ...
                                change(
                                    "krakenPresent",
                                    "<b>Continue fishing. The kraken is still feeding elsewhere.</b>"
                                );
                            }
                        } else {
                            complete = true;
                        ...
                        boat.style.left =
                            targetElement.xpos * (400 / 11) + "px";
                        outcomeOpen = true;
                    } else {
                        // this is what happens when they don't click one of the 5 bonus tiles
                        alert(
                            "You need to pick one of the 5 highlighted tiles."
                        );
                    }
